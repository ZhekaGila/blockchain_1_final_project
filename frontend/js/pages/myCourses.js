...
AC.renderMyCourses = () => {
  ...
  const rows = my.map(x => {
    ...
    const status = (p >= 1)
      ...
      : `<span class="tag warn">⏳ ${AC.percent(p)}</span>`;

    const showCompleteOnChain = (p >= 1) && !x.onchainCompleted;
    const canClaim = (p >= 1) && !!x.onchainCompleted && !AC.hasCertificate(x.id);

    const actions =
      `<button onclick="AC.openCourse('${x.id}')">Open</button>` +
      `<button class="btnOk" onclick="AC.addProgress('${x.id}', 0.25)">+25%</button>` +
      (showCompleteOnChain
        ? `<button class="btnWarn" onclick="AC.completeCourseOnChain('${x.id}')">Complete on-chain</button>`
        : ``) +
      (canClaim
        ? `<button class="btnPrimary" onclick="AC.claimCertificate('${x.id}')">Claim certificate</button>`
        : ``);
    ...
    return `
      <tr>
        ...
        <td><div class="rowBtns">${actions}</div></td>
      </tr>
    `;
  }).join("");
